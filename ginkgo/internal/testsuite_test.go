...
var _ = Describe("TestSuite", func() {
	Describe("Finding Suites", func() {
		...
		BeforeEach(func() {
			...
			writeFile("/ignored", ".ignore_dot_test.go", `import "github.com/danp60/ginkgo/v2"`, 0666)
			writeFile("/ignored", "_ignore_underscore_test.go", `import "github.com/danp60/ginkgo/v2"`, 0666)
			...
			writeFile("/colonelmustard", "colonelmustard_test.go", `import "github.com/danp60/ginkgo/v2"`, 0666)

			//ginkgo tests in a deeply nested directory
			writeFile("/colonelmustard/library", "library_test.go", `import "github.com/danp60/ginkgo/v2"`, 0666)

			//ginkgo tests in a deeply nested directory
			writeFile("/colonelmustard/library/spanner", "spanner_test.go", `import "github.com/danp60/ginkgo/v2/dsl/core"`, 0666)

			//ginkgo tests deeply nested in a vendored dependency
			writeFile("/vendor/mrspeacock/lounge", "lounge_test.go", `import "github.com/danp60/ginkgo/v2"`, 0666)
