...
func (f FixtureManager) copyAndRewrite(src string, dst string) {
	...
	for _, file := range files {
		...
		srcContent = bytes.ReplaceAll(srcContent, []byte("github.com/danp60/ginkgo/v2/integration/_fixtures"), []byte(f.PackageRoot()))
...
func (f FixtureManager) PackageRoot() string {
	return "github.com/danp60/ginkgo/v2/integration/" + f.TmpDir
}
